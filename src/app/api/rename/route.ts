...
export async function POST(request: Request) {
  ...
  if (!oldName || !newName) {
    return NextResponse.json({ message: 'Old name and new name are required' }, { status: 400 })
  }
  ...
  try {
    ...
    if (newFileExists) {
      return NextResponse.json({ message: 'A file with the new name already exists' }, { status: 409 })
    }
